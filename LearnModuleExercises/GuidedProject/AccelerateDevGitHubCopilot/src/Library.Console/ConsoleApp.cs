...
public class ConsoleApp
{
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        // Find a BookItem (a physical copy) for the book
        var bookItem = _jsonData.BookItems?.FirstOrDefault(bi => bi.BookId == book.Id);
        if (bookItem == null)
        {
            Console.WriteLine($"{book.Title} has no copies in inventory.");
            return ConsoleState.PatronDetails;
        }

        // Check loans to see if that BookItem is currently on loan (ReturnDate == null)
